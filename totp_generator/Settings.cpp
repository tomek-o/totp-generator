...
int Settings::Read(AnsiString asFileName)
{
	...
	{
		const Json::Value &jTotp = root["Totp"];
		if (jTotp.type() == Json::objectValue)
		{
			jTotp.getAString("Password", Totp.password);
			jTotp.getBool("StorePasswordInFile", Totp.storePasswordInFile);
			jTotp.getBool("RefreshAutomatically", Totp.refreshAutomatically);
		}
	}
...
int Settings::Write(AnsiString asFileName)
{
	...
	{
		Json::Value &jTotp = root["Totp"];
		jTotp = Json::Value(Json::objectValue);
		if (Totp.storePasswordInFile)
			jTotp["Password"] = Totp.password;
		jTotp["StorePasswordInFile"] = Totp.storePasswordInFile;
		jTotp["RefreshAutomatically"] = Totp.refreshAutomatically;
	}
